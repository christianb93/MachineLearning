...
$$
that depends only on temperature and pressure and on the reaction itself. To relate this constant $K$ to Gibbs energies, let us introduce the notation
$$
...
or
$$
K = \exp (- \frac{\Delta_r G}{R T})
$$
in aligment with section chapter 5 of \cite{Schroeder}.
But what exactly is $\Delta_r G$? To see this, let us again consider a simple example - a reaction of the form 
$$
A + B \longrightarrow C
$$
The constant $\Delta_r G$ is then, by definition, given by
$$
\Delta_r G = \mu_C - (\mu_A + \mu_B)
$$
Now $\mu_A + \mu_B$ is the Gibbs free energy of a solution containing one mole of $A$ and one mole of $B$, and $\mu_C$ is the Gibbs free energy of one mole of $C$. Thus the difference is the change in the Gibbs free energy if during the reaction one mole of $A$ and one mole of $B$ is converted into one mole of $C$, assuming that during the entire process, the chemical potentials remain constant (i.e we ignore the change in concentration). We therefore find that $\Delta_r G$ is the change in the Gibbs energy caused by the reaction at standard conditions (and approximately constant concentration).
So the equilibrium constant and the of the Gibbs energy during a reaction are related by


Let us pause for a moment and take a look at the dependency of the chemical potential on the concentration. We know by the above relation between the chemical potential and the concentration that at standard conditions ($c = 1$), the derivative of the chemical potential with respect to the concentration is $RT$ which is in the order of $2.5 kJ \cdot \text{mol}^{-1}$. For many reactions, the chemical potentials are in the order of a few hundred $kJ \cdot \text{mol}^{-1}$. Therefore the change of the chemical potential with a changing concentration can be neglected. 

In general, we could still {\em define} $\Delta_r G$ by the difference of the weighted chemical potentials of products and reactants, and this is in fact how this quantity is determined in practice - we look up the individual chemical potentials in some tables, which are usually called molar free Gibbs energy in chemistry textbooks, and add them up, taking stoichiometric coefficients into account. However, the identification with the actual change in Gibbs energy during the reaction is then only valid if the concentrations are close to the standard value (usually $1$) throughout the reaction, which has to be ensured by the setup of the experiment (for instance because we let actually only a small fraction of a mole react and then scale up, which we can do as the Gibbs free energy is an extensive quantity).

We also see that if the Gibbs free energy of the reaction is negative, the quilibrium constant tends to be large so that the reaction will run almost to completion, while if the Gibbs free energy is positive, the equilibrium will be on the side of the reactants. 
