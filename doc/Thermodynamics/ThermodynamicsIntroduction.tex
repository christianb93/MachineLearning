...
\begin{figure}[ht]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Energy niveaus in the Boltzmann distribution}
...
\end{cases}
$$
and a constant $J$ that describes the size of the interaction. It is common to use the notation $\langle i, j \rangle$ for a pair of lattice points that are direct neighbors. With this convention, our formula reads
$$
...
To prepare for our discussion of the Ising model, let us first consider the case that $J = 0$, i.e. there is no interaction, and let us try to compute the partition function for this simple case. As now
$$
E(s) = - B \sum_i s_i 
$$
we have
...
&= \sum_s \exp(\beta B \sum_i s(i) ) = \sum_s \exp(\beta B N m(s) )
\end{align*}

This looks daunting, but recall that there is no interaction - the energy that we consider is the sum of the individual contributions $-B s_i$ for each site $i$, so we can try to exploit the fact that the partition function is multiplicative. To do this, we first need to calculate the partition function $Z_1$ for the case $N = 1$. This is easy, as there are only two possible states:
$$
Z_1 = \sum_{s = -1, 1} \exp(\beta B s) ) = \exp(- \beta B) + \exp(\beta B) = 2 \cosh(\beta B)
$$
With this and the fact that the partition function is multiplicative, we immediately obtain the expression
$$
Z = Z_1^N = 2^N \cosh(\beta B)^N
$$
for the case $J = 0$.

Of course, the situation is much more complicated in the case $J \neq 0$, and it appears hopeless to find a closed expression for the general model (there are, however, closed expressions for the case $d=1$ due to Ising himself and for the case $B = 0$ and $d = 2$ due to Onsager). However, there is an approximation method that already displays some of the real behaviour of the model. To motivate this method, let us spell out the interaction in terms of the average magnetization $M$, or, more precisely, on the deviations $s_i - M$ that reflect the fluctuations of the spins. Now trivially
\begin{align*}
...
so that
\begin{align*}
\langle m \rangle &= \sum_s p(s) m(s) \\
&= \frac{1}{Z} \frac{1}{N} \sum_s  \exp(- \beta E(s))  \sum_i s_i\\
&=  \frac{1}{\beta} \frac{1}{Z} \frac{1}{N} \sum_s \exp(- \beta E(s)) \frac{\partial}{\partial B} (- \beta E(s)) \\
&= \frac{1}{N \beta} \frac{1}{Z} \frac{\partial}{\partial B} \sum_s   \exp(- \beta E(s)) \\
&= \frac{1}{N \beta} \frac{1}{Z} \frac{\partial}{\partial B} Z = \frac{1}{N \beta} \frac{\partial}{\partial B} \ln Z
\end{align*}
