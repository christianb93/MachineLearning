...
Another quantity that is of interest and that we will meet again later is magnetization of our system, i.e. the average $\langle s\rangle$ of the spin. This is again easy to calculate
\begin{align*}
\langle s \rangle &= p(s = 1) - p(s = -1) \\
...
$$
\langle s \rangle = - \frac{\partial F}{\partial B}
$$
...
Let us now go back to the Ising model and, using a slightly more neutral notation, try to carry out this approach. Of course, the reference system we want to choose is a two-state system without interaction. Thus we set
$$
E_0 = - (\Lambda + B) \sum_i s_i
$$ 
where again $\Lambda$ is the model parameter that we will later have to fit. Then
$$
E_1 = E - E_0 = - J \sum_{\langle i, j\rangle} s_i s_j  +  \Lambda  \sum_i s_i
$$
The Helmholtz energy of our model system is then (again, this is a two-state system) by
$$
F_0 = - k_B T N \ln 2 \cosh(\beta (\Lambda + B))
$$
...
$$
\Lambda \sum_i \langle s_i \rangle_0 = \Lambda N M_0 
$$
...
$$
\langle E_1 \rangle_0 = - \frac{1}{2}J N M_0^2 q + \Lambda N M_0 
$$
so that our variational Helmholtz potential is now
$$
F_v = F_0 - \frac{1}{2}J N M_0^2 q + \Lambda N M_0 
$$
The next step is to minimize this with respect to $\Lambda$. When taking the derivatives, we need to keep in mind that $M_0$ itself depends on $\Lambda$. To calculate the derivative of $F_0$ with respect to $\Lambda$, we can note that $\Lambda$ appears in $F_0$ in the same way as $B$, so that the derivative is equal to that with respect to $B$ which is minus the total magnetization:
$$
\frac{dF_0}{d\Lambda} = \frac{dF_0}{dB} = - N M_0
$$
Taking the derivatives of the other terms and applying the product rule yields
$$
\frac{dF_v}{d\Lambda} = - N M_0 -  J N q M_0 M_0'   + N M_0  + \Lambda N M_0'
$$
Some of the terms cancel, and we obtain that
$$
\frac{dF_v}{d\Lambda} =  N M_0' (\Lambda -  J  q M_0) 
$$
...
$$
and this is a strictly monotonous function. Thus we can conclude that the derivative of $F_v$ with respect to $\Lambda$ is zero if and only if
$$
\Lambda =  J q M_0(\Lambda)
$$
where we have added the explicit dependency of $M_0$ on $\Lambda$ in the notation. Thus we again obtain a self-consistency condition - our variational Helmholtz potential is the best possible approximation if and only if this condition holds.

Before we turn to a physical interpretation of this result and relate it to what we have found earlier, let us try to calculate the real magnetization - recall that $M_0$ is the magnetization of the reference system, not that of the true system. We do not know the exact Helmholtz potential $F$, but we know that it is the Helmholtz potential coming from a Boltzmann distribution with energy $E(s)$, so on general grounds, we know that the true magnetization is minus the partial derivative of the Helmholtz potential by $B$. So let us try to approximate this by taking the corresponding derivative for our variational Helmholtz potential.

To calculate this, we can argue as before, as $F_0$ and $M_0$ depend on $B$ in the same way as on $\Lambda$. We get
$$
M = \frac{\partial F_v}{\partial B} = - N M_0 -  J N q M_0 M_0' + \Lambda N M_0' = - N M_0 + N M_0 ' (\Lambda -  J q M_0)
$$
This gives us a different interpretation of our consistency condition - this is exactly the condition that we need in order for the magnetization that we obtain from the variational Helmholtz potential to be the same as $M_0$. In other words, the consistency condition ensures that the approximation of $M$ to order zero - which is $M_0$ - is the same as the approximation to first order (the derivative of $F_v$). 

This now gives us the physical interpretation we are looking for. If the consistency condition holds, then the variational Helmholtz potential is
\begin{align*}
F_v &= F_0 - \frac{1}{2}J N M^2 q + \Lambda N M_0  \\
&= - k_B T N \ln 2 \cosh (J q M + B) - \frac{1}{2}J N M^2 q + J q N M^2 \\
&= - k_B T N \ln 2 \cosh (J q M + B) + \frac{1}{2}J N M^2 q
\end{align*}
This is exactly the expression that we have obtained earlier by starting with an energy term
$$
E(s)  = \frac{1}{2} J q N M^2  - (J M q + B) \sum_i s_i
$$
So the parameter $\Lambda$ now appears - up to the constant factor $J q$ - as the mean field which we obtained in our more heuristic derivation earlier. Morever, spelling out the consistency condition using our explicit expression for $M_0$, reads
$$
\Lambda = J q \tanh(\beta(\Lambda + B))
$$ 
which is, again up to the constant factors, the same condition that we have obtained in the previous section. Thus the variational approach leads to the same mean field approximation and the same consistency as before. 
