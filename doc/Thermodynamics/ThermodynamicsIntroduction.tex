...
Let us now make our example more concrete by assuming a specific relation between entropy and the extensive parameters which holds for both subsystems. Specifically, let us assume that our system consists of two chambers, both filled with an ideal gas, with particle numbers $N_1$ and $N_2$, so that the energy of each subsystem is given by
$$
U_i = c N_i R T_i
$$
...
It is instructive to study a graphical plot of a typical entropy and visualize how and why both principles - maximal entropy and minimal energy - work and yield the same result\footnote{This is in fact a Python generated version of an illustration in \cite{Callen} that actually uses the example that we have just discussed}.
...
Typically, we think of the reservoir as being connected to the system by a diathermal, but fixed wall that encloses the system under consideration completely. Then the volume of the system that we consider is fixed, and neither the system itself nor the heat bath can do any work - just think of a process taking place in a vessel immersed into a heat bath. In that situation, the overall energy is clearly conserved, and we can apply our principle. This is why it is often stated that the Helmholtz energy is minimized under the constraint of constant temperature. 

\begin{figure}[ht]
	\begin{center}
	\includegraphics[scale=.5]{SpringLoadedPiston}
	\caption{A piston loaded by a spring in a heat bath}
	\label{fig:SpringLoadedPiston}
	\end{center}
\end{figure}


However, the condition of fixed volume is not really necessary, as long as the overall system is sufficiently isolated. To illustrate this, let us look at an example taken from \cite{Schwartz}. Imagine a cylinder containing an ideal gas that is, via a piston, in contact with a spring and fully immersed in a heat bath, as shown in diagram \ref{fig:SpringLoadedPiston}.
...
dG = - T dS_{tot}
$$
Note that we have also used that $T dS_{PR} = 0$, i.e. that the pressure reservoir has a constant entropy. As for the Helmholtz energy, this implies that the Gibbs energy has a minimum in the equilibrium state. More general, for systems that are in touch with a heat and a pressure reservoir, the Gibbs energy is minimized by the equilibrium state
...
This is now where the lack of convexity comes into play - this point is located {\em below} the point $M$, i.e. has a lower Helmholtz energy than $M$. Now recall that the Helmholtz energy is minimized in the presence of a heat bath and at fixed volume. As we have found a thermodynamic state $N$ - our mixed state  - that occupies the same volume as $M$ but has a lower Helmholtz energy, we can therefore conclude that $M$ is not stable. Moreover, this state is stable, as we have asumed that our states $A$ and $B$ are on the coexistence curve, i.e. equalities of pressure and molar Gibbs energy hold. We can conclude that, if we perturb $M$ a bit, it will move to state $N$ and remain there.  
